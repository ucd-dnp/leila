...
def col_tipo(base, detalle="bajo"):
    ...
    :param detalle: (str) {'bajo', 'alto'}, valor por defecto: 'bajo'. Indica el nivel de detalle en la descripción del tipo.
...
def unicos(base, faltantes=False):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param faltantes: (bool) {True, False}, valor por defecto: False. Indica si desea tener en cuenta los valores faltantes en el conteo de valores únicos.
...
def faltantes(base, porc=True):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param porc: (bool) {True, False}, valor por defecto: True. Si el valor es True el resultado se expresa como un porcentaje, si el valor es False el valor se expresa como una cantidad de registros (número entero).
...
def extremos(base, extremos="ambos", porc=True):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param extremos: (str) {'superior', 'inferior', 'ambos'}, valor por defecto: 'ambos'. Si el valor es '**inferior**' se tienen en cuenta los registros con valor menor al límite inferior calculado por la metodología de valor atípico por rango intercuartílico. Si el valor es '**superior**' se tienen en cuenta los registros con valor mayor al límite superior calculado por la metodología de valor atípico por rango intercuartílico. Si el valor es '**ambos**' se tienen en cuenta los registros con valor menor al límite inferior calculado por la metodología de valor atípico por rango intercuartílico, y también aquellos con valor mayor al límite superior calculado por la metodología de valor atípico por rango intercuartílico.
    :param porc: (bool) {True, False}, valor por defecto: True. Si el valor es True el resultado se expresa como un porcentaje, si el valor es False el valor se expresa como una cantidad de registros (número entero).
...
def descriptivas(base, float_transformar=False):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param float_transformar: (bool) {True, False}, valor por defecto: True. Si el valor es True se intenta realizar una transformación de valores de texto a numérico (float) para ser incluidas en el análisis, si el valor es False no se intenta realizar la transformación.
...
def varianza_percentil(base, percentil_inferior=5, percentil_superior=95, float_transform=False):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param percentil_inferior: (float), valor por defecto: 5. Percentil inferior de referencia en la comparación.
    :param percentil_superior: (float), valor por defecto: 95. Percentil superior de referencia en la comparación.
...
def categorias(base, limite=0.5, transformar_nums=False, variables=None):
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param limite: (float) (valor de 0 a 1) límite de referencia, se utiliza para determinar si las variables posiblemente son de tipo categóricas y ser incluidas en el análisis. Si el número de valores únicos por columna es mayor al número de registros * limit, se considera que la variable no es categórica.
    :param transformar_nums: (bool) {True, False}, determina si se desea considerar las variables como categóricas e incluirlas en el análisis. Si el valor es True se incluyen las variables numéricas en el análisis, si el valor es False no se incluyen las variables numéricas en el análisis.
...
def resumen_base(base, filas=True, columnas=True, col_numericas=True, col_texto=True,
    ...
    :param base: (dataframe) base de datos de interés a ser analizada.
    :param filas: (bool) {True, False}, valor por defecto: True. Indica si se incluye el cálculo de número de filas del dataframe.
    :param columnas: (bool) {True, False}, valor por defecto: True. Indica si se incluye el cálculo de número de filas del dataframe.
    :param col_numericas: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas de tipo numéricas.
    :param col_texto: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas de tipo texto.
    :param col_booleanas: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas de tipo boolean.
    :param col_fecha: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas de tipo fecha.
    :param col_otro: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas de otro tipo deferente a los anteriores.
    :param filas_nounicas: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de filas no únicas.
    :param columnas_nounicas: (bool) {True, False}, valor por defecto: False. Indica si se incluye el número de columnas no únicas.
    :param col_faltantes: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas con más de la mitad de las observaciones con datos faltantes.
    :param col_extremos: (bool) {True, False}, valor por defecto: True. Indica si se incluye el número de columnas con más del 10% de observaciones con datos extremos.
    :param memoria_total: (bool) {True, False}, valor por defecto: True. Indica si se incluye el cálculo del tamaño de la base de datos en memoria.
